  ...
  return <Container>
    <OtherNews>
      {data.map((item: any) => <OtherNewsItem href={`news/${item.id}`} key={item.id}>
        <OtherNewsImage src={`https://admin.mp60.ru${item.attributes.image.data.attributes.url}`} width='100' height='328' alt='news image'/>
        <OtherContent>
          <OtherNewsMeta>
          ...
          <OtherNewsText>{`${parse(item.attributes.text.replace(/<(.|\n)*?>/g, '').substring(0, 200))}...`}</OtherNewsText>
          {console.log(item.attributes.text)}
        </OtherContent>
      </OtherNewsItem>
      )}
    </OtherNews>
